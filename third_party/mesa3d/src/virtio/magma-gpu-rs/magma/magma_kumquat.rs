...
pub struct MagmaKumquat {
    ...
    pub _caps: MagmaVirtCapabilities,
...
impl MagmaKumquat {
    pub fn new() -> MagmaGpuResult<MagmaKumquat> {
        ...
        // TODO: A fixed buffer size is a temporary hack until VIRTGPU_PARAM_CAPSET_SIZE is available.
...
impl GenericPhysicalDevice for MagmaKumquat {
    ...
    fn query_memory_types(&self) -> MagmaGpuResult<Vec<MagmaMemoryType>> {
        self.devices
            .first()
            .map(|d| d.memory_types.clone())
            .ok_or(MagmaGpuError::Unsupported)
    }

    fn query_memory_heaps(&self) -> MagmaGpuResult<Vec<MagmaHeap>> {
        self.devices
            .first()
            .map(|d| d.memory_heaps.clone())
            .ok_or(MagmaGpuError::Unsupported)
    }

    fn query_queue_family_properties(&self) -> MagmaGpuResult<Vec<MagmaQueueFamilyProperties>> {
        let dev = self.devices.first().ok_or(MagmaGpuError::Unsupported)?;
        let count = dev.info.queue_family_count as usize;
        Ok(dev.info.queue_families[..count.min(dev.info.queue_families.len())].to_vec())
    }
}

impl GenericDevice for MagmaKumquat {
    fn get_memory_budget(&self, heap_idx: u32) -> MagmaGpuResult<MagmaHeapBudget> {
        // TODO: A GET_MEMORY_BUDGET protocol message is needed.
        let dev = self.devices.first().ok_or(MagmaGpuError::Unsupported)?;
        if let Some(heap) = dev.memory_heaps.get(heap_idx as usize) {
            Ok(MagmaHeapBudget {
...
pub fn enumerate_devices() -> MagmaGpuResult<Vec<MagmaPhysicalDevice>> {
    ...
    for dev in &enc.devices {
        devices.push(MagmaPhysicalDevice::new(enc.clone(), dev.info));
    }
