...
impl GenericPhysicalDevice for I915PhysicalDevice {
    ...
    fn query_memory_types(&self) -> MagmaGpuResult<Vec<MagmaMemoryType>> {
        i915_query_memory(self.descriptor.as_fd()).map(|(types, _)| types)
    }

    fn query_memory_heaps(&self) -> MagmaGpuResult<Vec<MagmaHeap>> {
        i915_query_memory(self.descriptor.as_fd()).map(|(_, heaps)| heaps)
    }
...
fn i915_query_memory(fd: BorrowedFd<'_>) -> MagmaGpuResult<(Vec<MagmaMemoryType>, Vec<MagmaHeap>)> {
...
impl I915 {
    pub fn new(physical_device: Arc<dyn PhysicalDevice>) -> MagmaGpuResult<I915> {
        ...
        let (_mem_types, mem_heaps) = i915_query_memory(fd)?;
