...
fn panthor_query_memory_heaps() -> Vec<MagmaHeap> {
...
fn panthor_query_memory_types(gpu_info: &drm_panthor_gpu_info) -> Vec<MagmaMemoryType> {
...
impl GenericPhysicalDevice for PanthorPhysicalDevice {
    ...
    fn query_memory_types(&self) -> MagmaGpuResult<Vec<MagmaMemoryType>> {
        Ok(panthor_query_memory_types(&self.gpu_info))
    }

    fn query_memory_heaps(&self) -> MagmaGpuResult<Vec<MagmaHeap>> {
        Ok(panthor_query_memory_heaps())
    }
...
impl Panthor {
    pub fn new(physical_device: Arc<PanthorPhysicalDevice>) -> MagmaGpuResult<Panthor> {
        let mem_types = physical_device.query_memory_types()?;
        let mem_heaps = physical_device.query_memory_heaps()?;
        Ok(Panthor {
            physical_device,
            mem_types,
            mem_heaps,
...
impl PanthorBuffer {
    fn new(
        ...
        let mem_type = mem_types
            ...
            .ok_or(MagmaGpuError::WithContext("invalid arguments"))?;
        let mut flags = 0u32;
