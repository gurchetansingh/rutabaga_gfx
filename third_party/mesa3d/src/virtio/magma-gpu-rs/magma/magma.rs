...
impl MagmaPhysicalDevice {
    ...
    pub fn get_memory_types(&self) -> MagmaResult<Vec<MagmaMemoryType>> {
        let mem_types = self.physical_device.query_memory_types()?;
        Ok(mem_types)
    }

    pub fn get_memory_heaps(&self) -> MagmaResult<Vec<MagmaHeap>> {
        let mem_heaps = self.physical_device.query_memory_heaps()?;
        Ok(mem_heaps)
    }

    pub fn get_queue_family_properties(&self) -> MagmaResult<Vec<MagmaQueueFamilyProperties>> {
        let queue_families = self.physical_device.query_queue_family_properties()?;
        Ok(queue_families)
    }
